...
from datas.login_data import login_error_data, login_success_data, login_unauthorized_data
...
class TestLogin:
    ...
    @pytest.mark.error
    @pytest.mark.parametrize('login_data', login_unauthorized_data)
    def test_unauthorized_login(self, login_data, init_web_login):
        driver, login_page = init_web_login
        try:
            login_page.login(login_data['phone'], login_data['pwd'])
            error_msg_element = login_page.get_unauthorized_msg
            assert login_data['expect'], error_msg_element.text
        except (TimeoutException, NoSuchElementException) as e:
            raise e
